...
- 🎯 Entry points to execute your program using `python -m <fast_pi>` or `$ fast_pi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/mrather/fast-pi/branch/main/graph/badge.svg?token=fast-pi_token_here)](https://codecov.io/gh/mrather/fast-pi)
[![CI](https://github.com/mrather/fast-pi/actions/workflows/main.yml/badge.svg)](https://github.com/mrather/fast-pi/actions/workflows/main.yml)

Awesome fast_pi created by mrather

## Install

from source
```bash
git clone https://github.com/mrather/fast-pi fast_pi
cd fast_pi
make install
```
...
```bash
$ fast_pi run --port 8080
```

or

```bash
python -m fast_pi run --port 8080
```
...
```bash
❯ fast_pi --help
Usage: fast_pi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_pi create-user --help
Usage: fast_pi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_pi create-user admin admin --superuser
```
...
Run with `fast_pi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_pi create-user`.
...
-----------------------------------------------------
fast_pi/__init__.py              4      0   100%
fast_pi/app.py                  16      1    94%
fast_pi/cli.py                  21      0   100%
fast_pi/config.py                5      0   100%
fast_pi/db.py                   10      0   100%
fast_pi/models/__init__.py       0      0   100%
fast_pi/models/content.py       47      1    98%
fast_pi/routes/__init__.py      11      0   100%
fast_pi/routes/content.py       52     25    52%
fast_pi/routes/security.py      15      1    93%
fast_pi/routes/user.py          52     26    50%
fast_pi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fast_pi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_pi/default.toml.

### As environment variables
```bash
export fast_pi_KEY=value
export fast_pi_KEY="@int 42"
export fast_pi_KEY="@jinja {{ this.db.uri }}"
export fast_pi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fast_pi_ENV=production fast_pi run
```
